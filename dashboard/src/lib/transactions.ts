...
export async function getTransactions(userId: string, startDate: string, endDate: string): Promise<DashboardTransaction[]> {
  ...
  const base = [where('telegramUserId', '==', userId), where('createdAt', '>=', start), where('createdAt', '<=', end), orderBy('createdAt', 'desc')];
  const [expenseSnapshot, incomeSnapshot] = await Promise.all([
    getDocs(query(collection(firestore, 'expenses'), ...base)),
    getDocs(query(collection(firestore, 'income'), ...base)),
  ]);
